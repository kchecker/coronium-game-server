...
-- @tparam[opt] int index_num A unique index number. *Optional*.
-- @treturn string A unique id.
-- @usage local uuid = tools.uuid()
function Tools.uuid( index_num )
    --Object UUID generator

	if Tools.index < 1 then
		Tools.index = 1000000
	end

	local i = Tools.index - 1

	local index = index or i
	local salt = Timer.now() .. "coronium" .. ( math.random( Timer.now() ) + index )
	local genId = Tools.md5( salt )
...
-- @treturn int UNIX timestamp.
-- @usage local now = tools.now()
function Tools.now()
	return Timer.now()
end

--- Create an MD5 hash.
-- @tparam string input_string The string to hash.
-- @treturn string The newly created MD5 hash.
function Tools.md5( input_string )
